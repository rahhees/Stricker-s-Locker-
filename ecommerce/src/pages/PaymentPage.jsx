...
import { CreditCard, Banknote, ArrowLeft, ShieldCheck, CheckCircle, ChevronRight, Lock, Wallet, Loader2, Shield } from "lucide-react";
...
function PaymentPage() {
  ...
  const handleCompleteOrder = async () => {
    ...
    try {
      ...
      // ---------- CASE: COD (With Enhanced Loading) ----------
      if (selectedPaymentMethod === "cod") {
        // Artificial delay to make the "Processing" look important and high-end
      ...
      // ---------- CASE: RAZORPAY ----------
        ...
        setIsProcessing(false); // Razorpay handles its own UI, so we disable our overlay here
        if (!window.Razorpay) {
        ...
        const options = {
          ...
          handler: async function (response) {
            try {
              setIsProcessing(true); // Re-enable for verification
              ...
              navigate("/confirmation", {
                replace: true,
                state: { order: { orderId, amount: total.toFixed(2), paymentMethod: 'Online', items: displayItems, shippingAddress: orderData?.shippingAddress } }
              });
            } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white pt-28 pb-20 px-4 font-sans relative">
      
      {/* --- PREMIMUM LOADING OVERLAY --- */}
      <AnimatePresence>
        {isProcessing && selectedPaymentMethod === "cod" && (
          ...
          >
            ...
            >
              ...
              <div className="absolute inset-0 border-4 border-t-red-600 rounded-full"></div>
            </motion.div>
            ...
            >
              <h2 className="text-3xl font-black italic uppercase tracking-tighter mb-2">
                Deploying <span className="text-red-600">Order</span>
              </h2>
              <div className="flex items-center justify-center gap-2 text-gray-500 uppercase font-bold text-[10px] tracking-[0.3em]">
                <Shield size={12} className="text-red-600" />
                <span>Securing Armory Data...</span>
              </div>
            </motion.div>
            ...
            <motion.div 
              ...
              className="absolute left-0 right-0 h-[2px] bg-red-600/10 shadow-[0_0_15px_rgba(220,38,38,0.5)] z-[-1]"
            />
          </motion.div>
        )}
      </AnimatePresence>
              ...
              <React.Fragment key={s.step}>
                <div className="flex flex-col items-center">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black transition-all duration-500 ${
                    s.step === 3 ? "bg-red-600 text-white shadow-[0_0_20px_rgba(220,38,38,0.4)] rotate-3" : "bg-red-600/20 text-red-500"
                  }`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 items-start">
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="space-y-4">
              ...
              <div 
                ...
                className={`group relative p-6 rounded-[2rem] border-2 cursor-pointer transition-all duration-300 backdrop-blur-xl flex items-center justify-between ${
                  selectedPaymentMethod === 'razorpay' ? 'bg-red-600/10 border-red-600 shadow-[0_0_30px_rgba(220,38,38,0.1)]' : 'bg-white/5 border-white/5'
                }`}
              >
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 font-medium">Credit/Debit Cards, UPI, NetBanking</p>
                  </div>
                </div>
              ...
              <div 
                ...
                className={`group relative p-6 rounded-[2rem] border-2 cursor-pointer transition-all duration-300 backdrop-blur-xl flex items-center justify-between ${
                  selectedPaymentMethod === 'cod' ? 'bg-red-600/10 border-red-600 shadow-[0_0_30px_rgba(220,38,38,0.1)]' : 'bg-white/5 border-white/5'
                }`}
              >
                <div className="flex items-center gap-5">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-6 pt-6">
              <button onClick={handleBack} className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-gray-500 hover:text-white transition-colors">
          ...
          <div className="space-y-6 lg:sticky lg:top-28">
            <div className="bg-white/5 border border-white/10 rounded-[2.5rem] p-8 backdrop-blur-xl">
                ...
                {displayItems.map((item) => (
                  <div key={item.id} className="flex items-center gap-4 bg-white/5 p-3 rounded-2xl border border-white/5">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-gray-500 font-black italic">QTY: {item.quantity}</p>
                    </div>
              ...
              <button
                ...
                className="w-full bg-red-600 hover:bg-red-700 text-white font-black italic uppercase tracking-[0.2em] py-5 rounded-2xl transition-all shadow-xl shadow-red-900/20 active:scale-95 disabled:opacity-50 flex items-center justify-center gap-3"
              >
                {isProcessing ? <Loader2 className="animate-spin" /> : <>Complete Order <ChevronRight size={18} /></>}
              </button>
            </div>
          </div>
        </div>
